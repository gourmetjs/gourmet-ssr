...
test("Only module-b should be compiled as a source module for `local` stage", t => {
  ...
  // `builder.runtime.server` is set to `node 8` for `local` stage which is high enough
  // not to transform `class` syntax in `module-b`.
  t.ok(/module\.exports = class ModuleA \{\};/.test(client), "client 'module-a' must be intact");
  t.ok(/module\.exports = function ModuleB\(\) \{\s+"use strict";\s+_classCallCheck\(this, ModuleB\);\s+\};/.test(client), "client 'module-b' must be compiled");
...
test("Both module-a & module-b should be compiled as source modules for `test` stage", t => {
  ...
  t.ok(/module\.exports = function ModuleA\(\) \{\s+"use strict";\s+_classCallCheck\(this, ModuleA\);\s+\};/.test(client), "client 'module-a' must be compiled");
  t.ok(/module\.exports = class ModuleB \{\};/.test(server), "server 'module-b' must be intact");
  t.ok(/module\.exports = function ModuleA\(\) \{\s+"use strict";\s+_classCallCheck\(this, ModuleA\);\s+\};/.test(server), "server 'module-a' must be compiled");
  t.ok(/module\.exports = class ModuleB \{\};/.test(server), "server 'module-b' must be intact");
